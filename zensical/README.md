# zensical Home Assistant Add-on

![Supports aarch64 Architecture][aarch64-shield] ![Supports amd64 Architecture][amd64-shield] ![Supports armhf Architecture][armhf-shield] ![Supports armv7 Architecture][armv7-shield] ![Supports i386 Architecture][i386-shield]

A Home Assistant add-on that generates and serves beautiful documentation using zensical. Supports both local folders and Git repositories as source.

## Features

- **zensical with Material Theme**: Beautiful, responsive documentation
...
- **Default Configuration**: Provides sensible defaults if no zensical.yml exists
...
- Private key path: Set via `ssh_key_path` (default: `/ssl/zensical_ssh_key`).
...
1. Create a folder in your Home Assistant config directory (e.g., `/config/zensical`)
2. Add your markdown files to the `docs/` subfolder
3. Optionally add a custom `zensical.yml` configuration
...
3. Copy the private key to Home Assistant (e.g., `/ssl/zensical_ssh_key`)
4. Set `source_type` to `"git"` and configure `git_url` and `ssh_key_path`
5. Start the add-on

### Default Files

If no `zensical.yml` exists in your source, a default configuration will be created with:
...
- zensical-material: Material Design theme
- zensical-mermaid2-plugin: Mermaid diagram support
- zensical-minify-plugin: HTML/CSS/JS minification
- zensical-awesome-pages-plugin: Enhanced navigation

### Adding Custom Plugins

Add plugins by updating `zensical.yml` and installing Python packages via `requirements.txt`.

Example `zensical.yml`:
...
```
/config/zensical/
├── zensical.yml          # zensical configuration (optional)
...
script:
  rebuild_zensical:
    ...
    description: "Manually rebuild the zensical documentation"
    icon: mdi:book-refresh
    sequence:
      - service: rest_command.zensical_rebuild
      - service: notify.persistent_notification
        data:
          title: "zensical Rebuild"
...
2. Create a new script with the `rest_command.zensical_rebuild` service call
...
2. **Build Failed**: Verify zensical.yml syntax and required files exist
