#!/usr/bin/env python3
"""Simple HTTP API for triggering zensical rebuilds."""
...
BUILD_COMMAND = ["/etc/cont-init.d/10-build-zensical"]
...
class RebuildController:
    ...
    def _run_build(self) -> None:
        ...
        try:
            print("[rebuild-api] Running build command: /etc/cont-init.d/10-build-zensical", flush=True)
...
class RebuildHandler(BaseHTTPRequestHandler):
    ...
    server_version = "zensicalRebuildAPI/1.0"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="zensical rebuild API server")
